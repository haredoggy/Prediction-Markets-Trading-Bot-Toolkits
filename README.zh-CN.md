...
选择 Rust 作为此工具包的编程语言有以下几个关键原因：

- **性能**：零成本抽象和内存安全，无需垃圾回收开销，使 Rust 成为高频交易操作的理想选择
- **可靠性**：Rust 的所有权系统和编译时保证可以防止整个类别的错误（空指针异常、数据竞争、内存泄漏），这些错误在交易系统中可能是灾难性的
- **并发性**：内置的 async/await 与 Tokio 为同时处理多个 API 调用、WebSocket 连接和订单执行提供了出色的并发原语
- **生态系统**：强大的区块链交互（Alloy）、HTTP 客户端（reqwest）和异步运行时（Tokio）生态系统
- **生产就绪**：Rust 对安全性和性能的关注使其非常适合可能出现真实资金损失的金融应用

### Polymarket 的转变：从延迟套利到跟单交易

2026 年 2 月上旬至中旬，Polymarket 做出了一个重大改变，重塑了交易格局：**他们移除了加密市场接受者（市价）订单的约 500 毫秒人工延迟**。这一改变悄然引入，旨在减少延迟套利，阻止利用延迟的机器人，并提高整体市场公平性和效率。

**影响：**

基于延迟的做市和微套利机器人（如 gabagool22 和类似的高频设置）严重依赖这种延迟。没有这种结构性优势，纯粹的 HFT 风格套利和短期加密市场的低风险价差套利变得困难得多或无利可图。传统的机器人策略，如快速接受者进入错误定价或做市方取消窗口，基本上消失了。

**新策略：**

与其争夺消失的微观低效率或从头重建复杂的做市策略，**跟单交易利用人类/智能钱包的 alpha** —— 来自经验证交易者的方向性信念、时机和仓位大小。这种方法：

- **跟随聪明资金**：跟踪已证明持续盈利能力的成功交易者钱包
- **捕捉市场时机**：受益于人类对何时进入/退出头寸的直觉
- **降低复杂性**：无需复杂的做市策略或延迟优化
- **随经验证交易者扩展**：随着识别更多成功的钱包，您可以分散跟单交易组合

### 为什么是跟单交易机器人？

跟单交易机器人是此工具包中第一个完全功能的机器人，因为：

1. **市场现实**：延迟移除后，跟单交易代表了最可行的自动化策略之一
2. **可访问性**：比复杂的套利策略更容易理解和配置
3. **风险管理**：您可以通过跟单百分比和钱包选择来控制风险
4. **透明度**：所有交易都被记录和可见，便于审计和改进
5. **基础**：跟单交易基础设施（仓位跟踪、订单执行）作为未来机器人类型的基础

此工具包旨在通过提供强大、可靠的跟单交易工具来帮助交易者适应新的 Polymarket 格局 —— 利用成功交易者的智慧，而不是在延迟军备竞赛中竞争。
